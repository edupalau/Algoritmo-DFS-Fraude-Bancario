...
def buscar_cadena_sospechosa_mas_larga(
    ...
    transacciones_maximas: int = 15, # <--- Agregamos un corte por defecto seguro
) -> dict | None:
    if grafo.number_of_nodes() == 0:
        return None
    ...
    for nodo_inicio in grafo.nodes:
        # CORRECCIÓN: Ahora el DFS sabe cuándo frenar el recorrido profundo
        for camino in dfs(grafo, str(nodo_inicio), edge_filter=filtro, max_depth=transacciones_maximas):
